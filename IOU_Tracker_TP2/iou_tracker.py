...
def main():
    ...
    for frame_num in range(1, num_frames + 1):
        ...
        if len(tracks) > 0 and len(current_detections) > 0:
            ...
            for r, c in zip(row_ind, col_ind):
                if cost_matrix[r, c] < 1 - iou_threshold:
                    tracks[r]['bbox'] = current_detections[c][:4]
                    tracks[r]['age'] = 0
        ...
        for det_idx in unmatched_detections:
            new_track = {
                'id': next_track_id,
                'bbox': current_detections[det_idx][:4],
                'age': 0
            }
    ...
    for frame_num in range(1, num_frames + 1):
        ...
        if video_writer is None:
            ...
            video_writer = cv2.VideoWriter(os.path.join(output_dir, f"{seq_name}.mp4"), fourcc, 15, (frame.shape[1], frame.shape[0]))
